      ...
      <main className="mx-auto max-w-4xl px-4 py-8">
        ...
        <div className="mb-10">
          <AudioProvider>
            <AudioDetailPlayer 
              id={data?.sys?.id} 
              audioSrc={`/api/audio?url=${data?.audio?.url}?t=${data?.audio?.sys?.publishedAt}&tag=audio:${data.audio?.sys?.id}`}
              downloadSrc={`/api/download?url=${data?.audio?.url}?t=${data?.audio?.sys?.publishedAt}&tag=audio:${data.audio?.sys?.id}`}
